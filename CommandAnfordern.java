...
public class CommandAnfordern implements Command {
	...
	public void execute(ApplicationContext context) {
		...
		// Abbrechen, falls keine Fuhrwerke mehr verf�gbar sind
		if(context.getVerfuegbareFuhrwerke()==0) {
			System.out.println("Es sind keine Fuhrwerke mehr fuer das Senden zu Niederlassungen verfuegbar.");
			// Verlassen des Menus
			return;
		}

		System.out.println(indexNdls + ": Zurueck");

		// Auswahl der Niederlassung, bei der anzufordern ist
		Scanner myScanner = new Scanner(System.in);
		while(!scannerErfolg) {
			try {
				System.out.println("Es koennen nur maximal "+context.getVerfuegbareFuhrwerke()+" Fuhrwerk gesendet werden.");
		...
		while(!scannerErfolg) {
			try {
				...
				if(context.getVerfuegbareFuhrwerke()>=anzahlZuSendenderFuhrwerke) {
					...
					System.out.println("Es koennen nur maximal "+context.getVerfuegbareFuhrwerke()+" gesendet werden.");
					scannerErfolg=false;
				}
				myScanner.nextLine();
			} catch (Exception e) {

...
public class CommandVerkaufen implements Command {
	...
	public void execute(ApplicationContext context) {
		...
		System.out.println("========== Welche der verfuegbaren Warenarten soll verkauft werden?");

		// Auswahlliste f�r die Warenart
		int indexWarenart;
		...
		System.out.println(indexWarenart + ": Zurueck");
		...
		while(!scannerErfolg) {
			try {
				...
				if(warenartMenge<=context.getLager().getBestand(ausgewaehlteWarenart)) {
					...
					System.out.println("Es koennen nur maximal "+context.getLager().getBestand(ausgewaehlteWarenart)+" Einheiten "+Util.convertWarenartToString(ausgewaehlteWarenart)+" verkauft werden.");
					scannerErfolg=false;
				}
				myScanner.nextLine();
			} catch (Exception e) {
